		...
		try {
			...
			for (Chauffeur chau : listeChauffeurs) {
				System.out.println("#- id:" + chau.getId().longValue() +
						",matricule:" + chau.getMatricule() + ", Nom:" + chau.getNom() + ", Prénom:" + chau.getPrenom());
			}

			
			// Tester recherche par Id
			System.out.println("le nom du chauffeur dont l'id est égal à 2 est :"
								+ chauffeurDao.rechercherChauffeurParId(2L).getNom());
